...
|---|---|---|
| `POSTGRES_USER` | Database username | `widtts` |
| `POSTGRES_PASSWORD` | Database password | `widtts_dev_password` |
| `POSTGRES_HOST` | Database hostname | `postgres` (Docker) / `localhost` (local) |
| `POSTGRES_PORT` | Database port | `5432` |
| `POSTGRES_DB` | Database name | `widtts` |
| `MASTER_ENCRYPTION_KEY` | Base64 32-byte AES-256 master key | *(required)* |
| `APP_SECRET` | Application session secret | `dev-secret-change-in-production` |
| `LIVEKIT_URL` | Browser/public-facing LiveKit URL | `ws://localhost:7880` |
| `LIVEKIT_INTERNAL_URL` | Backend container→LiveKit URL | `ws://livekit:7880` (Docker only) |
